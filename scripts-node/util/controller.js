...
import { SESSION_MODEL } from '../constants/model-constants';
...
export class Controller {
    ...
    for (var r in routing) {
      ...
      app.get(this.namespace() + r, (req, res) => {
        if (!req.body.hasOwnProperty('sid')) {
          ...
          return this.getSession(req.body.sid, req.body.account_num).then(
            data => {
              if (data !== null) {
                this.callRoute(method, req, res);
              } else {
  ...
  callRoute(method, req, res) {
    ...
    if (result && typeof result.then == 'function') {
      result.then(data => this.applyResult(req, res, data)).catch(e => {
